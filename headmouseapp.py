...
class HeadMouseApp(ctk.CTk):
    def __init__(self):
        ...
        self.proceso_ia = None # Variable para controlar el minijuego
    ...
    def cambiar_modo(self, nuevo_modo):
        ...
        self.log(f"Intentando cambiar perfil a: {nuevo_modo}")

        # === CASO 1: VAMOS HACIA EL ENTRENAMIENTO ===
        if nuevo_modo == "Entrenar IA":
            self.modo_actual = nuevo_modo # Pausamos el motor de fondo primero
            time.sleep(0.2) 
            
            self.lbl_status.configure(text="En Línea - MODO ENTRENAMIENTO", text_color="#3498DB")
            self.log("Liberando puerto USB...")
            ...
            try:
                ...
                self.proceso_ia = subprocess.Popen([sys.executable, ruta_entrenador])
                self.log("¡Juego abierto! Usa el recuadro rojo del juego para salir.")
            except Exception as e:
                self.log(f"Error al abrir: {e}")
                self.seg_modos.set("Escritorio")
                self.cambiar_modo("Escritorio") 

        # === CASO 2: VENIMOS DE REGRESO DEL ENTRENAMIENTO ===
        elif modo_anterior == "Entrenar IA":
            self.log("Recuperando el control del HeadMouse...")
            
            if self.proceso_ia and self.proceso_ia.poll() is None:
                self.proceso_ia.terminate()
            
            time.sleep(1) # Tiempo para que Windows libere el puerto COM
            
            try:
                # Re-instanciamos para evitar OSError 9
                self.arduino = serial.Serial(self.puerto_detectado, BAUD_RATE, timeout=0.1)
                time.sleep(1.5)
                self.arduino.flushInput()
                self.log("Puerto recuperado exitosamente.")
                
                self.modo_actual = nuevo_modo # Reactivamos el motor de fondo
                self.lbl_status.configure(text="En Línea - MODO ESCRITORIO", text_color="#2ECC71")
            except Exception as e:
                self.log(f"Error al reconectar puerto: {e}")

        # === CASO 3: CAMBIOS NORMALES ===
        else:
            ...
            if nuevo_modo == "Películas":
                ...
                self.lbl_status.configure(text="En Línea - MODO GAMING", text_color="#E74C3C")
    ...
    def iniciar_driver_mouse(self):
        try:
            ...
            while self.corriendo:
                # Si estamos calibrando o en IA, el hilo principal no hace nada
